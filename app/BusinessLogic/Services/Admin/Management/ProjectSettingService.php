...
use App\Models\Management\ProjectSetting;
use App\Models\User;
use App\Models\Admin\ErrorAndNotificationSystem;
use App\Models\Admin\Log;

/**
 * ProjectSettingService is a service class the will implement all the methods from the ProjectSettingInterface contract and will handle the business logic.
 */
class ProjectSettingService implements ProjectSettingInterface
{
    protected $modelName;
    protected $tableName;
    ...
    {
        $this->modelName          = new ProjectSetting();
    ...
    public function handleIndex()
    {
        try
        {
            $apiDisplayAllRecords = $this->modelName->select(
                'id', 'project_id', 'billable_rate', 'amount', 'budget', 'no_of_hours'
            )->get();
            ...
            if (Auth::check() && $this->getCurrentUserRole === 1 && $mysqlError->getCode() === '42S02') 
            {
                ...
                return response([
                    ...
                    'description'        => __('contact_me_system.store.err_00001_notify.user_has_rights.message_super_admin', [ 
                        'methodName'     => $_SERVER['REQUEST_METHOD'],
                        'apiEndpoint'    => $_SERVER['REQUEST_URI'],
                        'serviceName'    => __NAMESPACE__ . '\\' . basename(ProjectSettingService::class) . '.php',
                        'databaseName'   => config('database.connections.mysql.database'),
                        'tableName'      => 'clients',
                    ]),
                    'reference'          => config('app.url') . '/documentation/error#ERR_00001',

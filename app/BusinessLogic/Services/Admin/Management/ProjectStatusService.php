...
use App\Models\Management\ProjectStatus;
use App\Models\User;
use App\Models\Admin\ErrorAndNotificationSystem;
use App\Models\Admin\Log;

/**
 * ProjectStatusService is a service class the will implement all the methods from the ProjectStatusInterface contract and will handle the business logic.
 */
class ProjectStatusService implements ProjectStatusInterface
{
    protected $modelName;
    protected $tableName;
    ...
    {
        $this->modelName          = new ProjectStatus();
    ...
    public function handleIndex()
    {
        try
        {
            $apiDisplayAllRecords = $this->modelName->select('id', 'project_id', 'tracked_time', 'amount')->get();
        ...
        try
        {
            if (Auth::check() && $this->getCurrentUserRole === 1) 
            {
                $apiInsertSingleRecord = $this->modelName->create([
                    ...
                    'tracked_time' => $request->get('tracked_time'),
            ...
            if (Auth::check() && $this->getCurrentUserRole === 1 && $mysqlError->getCode() === '42S02') 
            {
                ...
                return response([
                    ...
                    'description'        => __('contact_me_system.store.err_00001_notify.user_has_rights.message_super_admin', [ 
                        'methodName'     => $_SERVER['REQUEST_METHOD'],
                        'apiEndpoint'    => $_SERVER['REQUEST_URI'],
                        'serviceName'    => __NAMESPACE__ . '\\' . basename(ProjectStatusService::class) . '.php',
                        'databaseName'   => config('database.connections.mysql.database'),
                        'tableName'      => 'clients',
                    ]),
                    'reference'          => config('app.url') . '/documentation/error#ERR_00001',

...
/**
 * ProjectService is a service class the will implement all the methods from the ProjectInterface contract and will handle the business logic.
 */
class ProjectService implements ProjectInterface
{
    ...
    public function handleIndex()
    {
        try
        {
            $apiDisplayAllRecords = $this->modelName->select('id', 'client_id', 'name')
            ->isActive()
            ->get();
            ...
            if (Auth::check() && $this->getCurrentUserRole === 1 && $mysqlError->getCode() === '42S02') 
            {
                ...
                return response([
                    ...
                    'description'        => __('contact_me_system.store.err_00001_notify.user_has_rights.message_super_admin', [ 
                        'methodName'     => $_SERVER['REQUEST_METHOD'],
                        'apiEndpoint'    => $_SERVER['REQUEST_URI'],
                        'serviceName'    => __NAMESPACE__ . '\\' . basename(ProjectService::class) . '.php',
                        'databaseName'   => config('database.connections.mysql.database'),
                        'tableName'      => 'clients',
                    ]),
                    'reference'          => config('app.url') . '/documentation/error#ERR_00001',

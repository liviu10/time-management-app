...
/**
 * UserListService is a service class the will implement all the methods from the UserListInterface contract and will handle the business logic.
 */
// TODO: copy translation links from ErrorAndNotificationSystemService
class UserListService implements UserListInterface
{
    protected $modelName;
    protected $tableName;
    ...
    public function __construct()
    {
        ...
        $this->getCurrentUserRole = $this->modelName::select('user_role_type_id')->where('user_role_type_id', '=', Auth::id())->get()->toArray()[0]['user_role_type_id'];
    }
    ...
    public function handleIndex()
    {
        try
        {
            $apiDisplayAllRecords = $this->modelName->select('id', 'name', 'nickname', 'email', 'user_role_type_id')
                                    ->with([
            ...
            if (Auth::check() && $this->getCurrentUserRole === 1) 
            {
                if ($apiDisplayAllRecords->isEmpty()) 
                {
                    ...
                    return response([
                        'title'              => __('dashboard_application.message_webmaster.index.user_list.title'),
                        'notify_code'        => 'WAR_00001',
                        'description'        => __('dashboard_application.message_webmaster.index.user_list.description', [ 
                            'methodName'     => $_SERVER['REQUEST_METHOD'],
                            'apiEndpoint'    => $_SERVER['REQUEST_URI'],
                            'serviceName' => __NAMESPACE__ . '\\' . basename(UserListService::class) . '.php',
                            'databaseName'   => config('database.connections.mysql.database'),
                            'tableName'      => $this->tableName 
                        ]),
    ...
    public function handleStore(UserListValidateRequest $request)
    {
        try
        {
            if (Auth::check() && $this->getCurrentUserRole === 1)
            {
                $checkIfRecordExists = $this->modelName->select('email')->where('email', '=', $request->get('email'))->get();
    ...
    public function handleUpdate(UserListValidateRequest $request, $id)
    {
        try
        {
            if (Auth::check() && $this->getCurrentUserRole === 1)
            {
                ...
                $getCurrentUserRoleType = $apiUpdateSingleRecord['name'];
        ...
        catch (\Illuminate\Database\QueryException $mysqlError)
        {
            ...
            if (Auth::check() && $this->getCurrentUserRole === 1 && $mysqlError->getCode() === '23000')
            {
                $this->modelName->log()->create([
                    'status'             => 'Error',
                    'status_description' => Auth::user()->name . ' failed to update a resource in ' . $this->tableName . ', because the HTTP request does not match the table structure.',
                    'request_details'    => __('error_and_notification_system.store.info_00002_notify.user_has_rights.message_super_admin', [
                        'record'         => null,
                        'databaseName'   => config('database.connections.mysql.database'),
                        'tableName'      => 'users',
